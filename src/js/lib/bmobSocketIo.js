...
/**
 * Based on JSON2 (//www.JSON.org/js.html).
 */
...
// Copyright: Hiroshi Ichikawa <//gimite.net/en/>
// License: New BSD License
// Reference: //dev.w3.org/html5/websockets/
// Reference: //tools.ietf.org/html/draft-hixie-thewebsocketprotocol
...
(function() {
  ...
  if (location.protocol == "file:") {
    console.error(
      ...
      "Open the page via Web server i.e. //...");
  }
  ...
  /**
   * Implementation of {@link <a href="//www.w3.org/TR/DOM-Level-2-Events/events.html#Events-registration">DOM 2 EventTarget Interface</a>}
   *
   * @param {string} type
  ...
  WebSocket.__initialize = function() {
    ...
    // //help.adobe.com/en_US/as3/mobile/WS4bebcd66a74275c36cfb8137124318eebc6-7ffd.html
    swfobject.embedSWF(
 ...
 */
(function(root) {
  root.BmobSocketIo = root.BmobSocketIo || {};
  root.BmobSocketIo.VERSION = "js0.0.1";
}(this));
...
(function(root) {
  root.BmobSocketIo = root.BmobSocketIo || {};
  /**
   * 包含了所有BmobSocketIo的api和函数
   *
   * 包含了所有BmobSocketIo的api和函数
   */
  var BmobSocketIo = root.BmobSocketIo;

   if (typeof(localStorage) !== "undefined") {
      BmobSocketIo.localStorage = localStorage;
    }
    if (typeof(XMLHttpRequest) !== "undefined") {
      BmobSocketIo.XMLHttpRequest = XMLHttpRequest;
    }
  ...
  if (typeof(process) !== "undefined" &&
    ...
    BmobSocketIo._isNode = true;
  }



  
  if (BmobSocketIo._isNode) {
    BmobSocketIo.initialize = BmobSocketIo._initialize;

  }

  BmobSocketIo._ajaxIE8 = function(method, url) {
    ...
    xdr.onload = function() {
      ...
      try {
        response = JSON.parse(xdr.responseText);
            //ajax获取ip and port, then set this value
            BmobSocketIo._setSocketIoIpAndPost(response.data.server);

            //conncet to socketio server
            BmobSocketIo._connectSocketIo();
      } catch (e) {
        throw "BmobSocketIo can not get socketio server ";
      }      
    };
  ...
  BmobSocketIo._ajax = function(method, url) {

    if (typeof(XDomainRequest) !== "undefined") {
      return BmobSocketIo._ajaxIE8(method, url);
    }

    var xhr = new BmobSocketIo.XMLHttpRequest();
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4) {
  ...
  //xhr.setRequestHeader("212312313", "32432342");  // avoid pre-flight.
    if (BmobSocketIo._isNode) {
      ...
      xhr.setRequestHeader("User-Agent",
                           "BmobSocketIo/" + BmobSocketIo.VERSION +
  ...
  BmobSocketIo.initialize = function(applicationId) {
    ...
    // Set the server for BmobSocketIo to talk to.
  BmobSocketIo.serverURL = "//io.bmob.cn:3010/";
